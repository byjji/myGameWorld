...
**NAS를 실제로 재부팅해 보고 확인한다.** 설정만 보고 넘어가면 정작 조카 집에 갔을 때
죽어 있다.
...
`proxy_read_timeout`을 60초보다 크게. 300초 정도.
클라이언트가 25초마다 ping을 보내지만(`js/net.js` `PING_MS`) 여유가 없으면 게임 중에 끊긴다.
이 증상은 30분 켜두는 시험에서만 드러난다.
