...
(function (global) {
  ...
  GP.tuning = {
    ...
    /* 점프 (PROJECT.md 4장) — 착지 스파이크 + 직전 체공 */
    ...
    // 스파이크 직전 이 시간(ms) 안에 자유낙하가 있었으면 착지 = 점프. 없으면 아무것도 아니다.
    // (쳐올리기는 이제 스파이크가 아니라 속도로 본다. 아래 "쳐올리기" 참고)
    // 착지 직후 같은 시간 동안은 쳐올리기·스쿼트 진입도 막는다 — 착지 반동은 동작이 아니다.
    ...
    // 처음엔 "체공 없는 가속도 스파이크 ≥ SPIKE_ON"으로 봤다. 실기에서 보통 속도로 쳐올리면
    // 12 m/s^2에 못 미쳐 안 잡히고, 대신 내려와 멈추는 왕복이 스쿼트로 잡혔다.
    // 지금은 스쿼트와 같은 잣대다: 세로 속도가 아래보다 먼저 위로 임계를 넘으면 쳐올리기.
    // 크기가 아니라 방향이 가른다.

    // 세로 속도(m/s)가 이 값을 위로 넘으면 쳐올리기. 30cm를 0.3초에 올리면 1 m/s 근처.
    // 폰을 얼굴 앞으로 천천히 드는 것(0.3~0.5)은 안 잡히게.
    PUNCH_VEL_UP: 0.6,

    // 그 순간 세로 가속도(m/s^2)도 이 값 이상. 표류 차단용 (SQUAT_ACCEL_DOWN과 같은 역할).
    PUNCH_ACCEL_UP: 3,

    // 한 번 잡힌 뒤 속도가 이 값 아래로 내려와야 다음 쳐올리기를 받는다.
    // 길게 밀어 올리는 동안 쿨다운마다 다시 잡히는 것을 막는다.
    PUNCH_VEL_REARM: 0.3,

    // 세기(p) 정규화. 잡히는 순간 가속도가 이 값이면 p=1.
    PUNCH_ACCEL_MAX: 20,
    ...
    // 낮게 둔 이유: 점프 앞의 작은 웅크림(-0.3 근처)도 여기 걸려야 도약이 쳐올리기로 안 보인다.
    // 잡음은 아래 가속도 게이트와 SQUAT_MIN_MS가 거른다.
    SQUAT_VEL_DOWN: 0.15,
    ...
    // 정지 판정. 세로 가속도 절대값이 이 값(m/s^2) 아래면 움직이지 않는 것으로 보고
    // 속도를 VEL_REST_TAU_MS로 빨리 0에 되돌린다. 안 움직이면 속도도 없다.
    // 이게 없으면 점프 몇 번 뒤 잔류 속도가 +0.3~0.4에 눌러앉아 다음 웅크림이 임계에 못 닿고,
    // 도약이 "위로 먼저"로 보여 쳐올리기가 된다.
    VEL_REST_ACCEL: 0.8,
    VEL_REST_TAU_MS: 150,

    // 일어나기 시작한 뒤 이 시간(ms) 동안 체공이 시작되지 않아야 스쿼트다.
    // 점프도 웅크렸다 일어나는 것으로 시작한다 — 차이는 그 직후 발이 뜨느냐뿐이다.
    // 도약 가속 구간이 어른 150~250ms라 그보다 살짝 길게. 스쿼트 발화가 이만큼 늦어진다.
    SQUAT_CONFIRM_MS: 250,

    SQUAT_COOLDOWN_MS: 300,

    // 스쿼트와 쳐올리기의 상호 배제 시간(ms). 힘차게 일어서면 가속도가 쳐올리기
    // 임계를 넘고, 쳐올린 폰이 가슴으로 돌아와 멈추면 세로 왕복이 생긴다.
    // 어느 쪽이든 먼저 잡힌 것이 이기고, 이 시간 동안 반대쪽은 잠근다.
    SQUAT_PUNCH_GUARD_MS: 800,
